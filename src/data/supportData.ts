...
export const trackItems: TrackItem[] = [
  {
    id: "tr-001",
    type: "project",
    title: "贵州茅台定增80亿方案推进项目",
    description: "华创-贵州茅台150亿定增方案，资管条线张明牵头客户分析，投行条线协助方案设计，涉及双条线协同及合规审批。定增募资主要用于酱香酒产能扩建与数字化营销转型。",
    relation: "owner",
    handler: "张明",
    handlerDept: "资管条线",
    progressLabel: "客户财务分析中",
    progressPercent: 35,
    updatedAt: "今日 10:20",
    status: "at_risk",
    hasRisk: true,
    riskDesc: "贵州茅台资产负债率连续2季度超75%，应付账款账期延长至90天，大额定增下公司偿债能力需重点评估",
    needsEscalation: true,
    escalationSubmitted: true,
    escalatedAgendaId: "ag-001",
    aiJudge: "财务指标异常需深度核查，相关议题已在协同端讨论（双条线策略协调），建议待协调结果产出后推进",
  },
  {
    id: "tr-002",
    type: "project",
    title: "资管模型指标体系重构项目（630节点）",
    description: "按资产负债/现金流/资产配置三维重构27个模型，精简冗余指标从200+降至80个核心指标，630前完成验收上线。涉及资管/投行/零售三条线的标准统一。",
    relation: "owner",
    handler: "温从余",
    handlerDept: "研发中心",
    progressLabel: "指标拆解中",
    ...
    status: "blocked",
    hasRisk: true,
    riskDesc: "630节点倒计时42天，当前进度滞后约10%，算力资源尚未到位，GPU扩容采购申请尚未获批",
    needsEscalation: true,
    escalationSubmitted: true,
    escalatedAgendaId: "ag-002",
    aiJudge: "算力和进度双瓶颈，算力申请已提请协同端审批（ag-002），建议同步启动预案：若算力延误，优先保障资产负债和现金流两维的模型上线",
  },
  {
    id: "tr-003",
    type: "project",
    title: "城商行债券承销与投顾服务 — 中信银行贵阳分行",
    description: "中信银行贵阳分行委托华创证券提供债券投资方案设计及后续承销服务，涉及资管/投行双条线配合，产品规模约30亿，分三期发行。",
    ...
    status: "at_risk",
    hasRisk: true,
    riskDesc: "方案现金流数据缺失（首期10亿授信池未确认），资产配置维度尚未填写，客户反馈窗口期仅剩5个工作日",
    needsEscalation: false,
    escalationSubmitted: false,
    aiJudge: "材料缺失明确，已提示补录。建议今日内与中信银行贵阳分行确认首期授信池及现金流安排，避免错过窗口期",
  },
  {
    id: "tr-004",
    type: "project",
    title: "华创-芯原半导体IPO保荐项目",
    description: "华创证券作为保荐机构推进芯原半导体科创板IPO，涉及电子信息产业的行业复核、财务尽调及合规审查。当前处于材料补录阶段，部分材料存在产业覆盖缺口。",
    relation: "participant",
    handler: "高瑾妮",
    handlerDept: "投行支持中心",
    progressLabel: "材料补录中",
    ...
    status: "at_risk",
    hasRisk: true,
    riskDesc: "缺少电子硬科技行业经验人员复核，现金流量表缺失，若本周内未补齐将错过本期申报窗口期",
    needsEscalation: false,
    escalationSubmitted: false,
    aiJudge: "材料缺口明确，现金流量表需客户方CFO提供，产业复核人员缺口建议协调新兴产业组李益丰团队支援",
  },
  {
    id: "tr-005",
    type: "task",
    title: "投行条线术语体系统一与歧义词清理",
    description: "落实5月18日会议精神（ag-005指令），统一投行四个核心模型的术语边界，清理现有文档中的歧义词汇，形成术语标准手册交付研发中心统一更新数据标准。",
    relation: "watcher",
    handler: "高瑾妮",
    handlerDept: "投行支持中心",
    progressLabel: "初稿撰写中",
    ...
    needsEscalation: false,
    escalationSubmitted: false,
    aiJudge: "进展正常，按ag-005指令节点（5月25日前）预计可按时完成，无需额外干预",
  },
]
